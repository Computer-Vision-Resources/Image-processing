...
//			pDst[c] = (pSrc[pos]+pSrc[pos+1]+pSrc[pos+2])*0.33333;
			pDst[c] = 0.114*pSrc[pos]+0.587*pSrc[pos+1]+0.299*pSrc[pos+2];  //녹색에 가중치를 둔 코드 !
			pos += 3;
		}
	}
...
template <typename T>
CMyImage<T> RGB2Gray_optionmode(const CMyImage<T>& src, int min, int max)
{
	ASSERT(src.GetChannel() == 3);
	...
	CMyImage<T> dst(nWidth, nHeight, 1);
	...
	for (int r=0 ; r<nHeight ; r++)
	{
		...
		for (int c=0 ; c<nWidth ; c++)
		{
			...
			else 
			{
				...
				if (vR==vMax)
					H = (vG-vB)/delta;		// 노란색과 자홍색 사이, R
				else if (vG==vMax)
					H = 2 + (vB-vR)/delta;	// 하늘색과 노란색 사이, G
				else
					H = 4 + (vR-vG)/delta;	// 자홍색과 하늘색 사이, B
